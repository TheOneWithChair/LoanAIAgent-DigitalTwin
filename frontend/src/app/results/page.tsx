    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-xl text-gray-700">Loading application results...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 mb-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-gray-600">
                Application ID: <span className="font-mono text-blue-600">{applicationData.applicant_id}</span>
              </p>
            </div>
            <button
              onClick={() => router.push("/application")}
          ...
          <div className={`bg-${decisionColor}-50 border-2 border-${decisionColor}-200 rounded-xl p-6 mb-6`}>
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-bold mb-2" style={{ color: decisionColor === "green" ? "#059669" : decisionColor === "red" ? "#DC2626" : "#D97706" }}>
                  {applicationData.final_decision ? applicationData.final_decision.toUpperCase() : "PENDING"}
                </h2>
                {applicationData.decision_explanation && (
                  <p className="text-gray-700">{applicationData.decision_explanation}</p>
                )}
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-blue-50 rounded-lg p-4">
              <p className="text-sm text-blue-600 font-medium mb-1">Loan Amount</p>
            ...
            {applicationData.monthly_payment && (
              <div className="bg-green-50 rounded-lg p-4">
                <p className="text-sm text-green-600 font-medium mb-1">Monthly Payment</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6">
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <p className="font-mono text-sm text-gray-900">{applicationData.applicant_id}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-900">{applicationData.phone_number}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Employment Status</p>
                  <p className="font-semibold text-gray-900">{applicationData.employment_status}</p>
                </div>
              </div>
            </div>
                    ...
                    <div>
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div
                          ...
                          style={{ width: `${(applicationData.analytics.credit_score / 850) * 100}%` }}
                        ></div>
                      </div>
                      {applicationData.analytics.credit_tier && (
                        <p className="text-xs text-gray-500 mt-1">{applicationData.analytics.credit_tier}</p>
                      )}
                    </div>
                  )}
                  ...
                  {applicationData.analytics.risk_level && (
                    <div>
                      ...
                      {applicationData.analytics.risk_score !== undefined && (
                        <p className="text-xs text-gray-500 mt-1">Risk Score: {applicationData.analytics.risk_score.toFixed(2)}</p>
                      )}
                    </div>
                  )}

                  {applicationData.analytics.approval_probability !== undefined && (
                    <div>
                      <div className="flex justify-between items-center mb-1">
                        <p className="text-sm text-gray-600">Approval Probability</p>
                        <p className="text-lg font-bold text-gray-900">
                          {(applicationData.analytics.approval_probability * 100).toFixed(1)}%
                        </p>
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div
                          className="bg-green-600 h-2 rounded-full transition-all"
                          style={{ width: `${applicationData.analytics.approval_probability * 100}%` }}
                        ></div>
                      </div>
                    </div>
                  )}

                  {applicationData.analytics.debt_to_income_ratio !== undefined && (
                    <div>
                      <div className="flex justify-between items-center">
                        <p className="text-sm text-gray-600">Debt-to-Income Ratio</p>
                        <p className="text-lg font-bold text-gray-900">
                          {(applicationData.analytics.debt_to_income_ratio * 100).toFixed(2)}%
                        </p>
                      </div>
                    </div>
                  )}
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-2xl shadow-xl p-6">
              ...
              {applicationData.agent_responses && applicationData.agent_responses.length > 0 ? (
                <div className="space-y-4">
                        ...
                        <div className="flex items-center">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-500">
                              Status: <span className={`font-semibold ${agent.status === "SUCCESS" ? "text-green-600" : "text-red-600"}`}>
                                {agent.status}
                              </span>
                              {agent.execution_time_ms && (
                                <span className="ml-2">• {agent.execution_time_ms}ms</span>
                              )}
                            </p>
                          </div>
                        </div>
                ...
                <p className="text-gray-500 text-center py-8">No agent responses available</p>
              )}
            </div>
                  ...
                  {applicationData.created_at && (
                    <div className="flex items-center">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{new Date(applicationData.created_at).toLocaleString()}</p>
                      </div>
                    </div>
                  )}
                  {applicationData.processed_at && (
                    <div className="flex items-center">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{new Date(applicationData.processed_at).toLocaleString()}</p>
                      </div>
                    </div>
                  )}
                </div>
